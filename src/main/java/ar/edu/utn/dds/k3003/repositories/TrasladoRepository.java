...
public class TrasladoRepository {
  ...
  public Traslado findById(Long id) {
   ...
   public List<Traslado> findCyColaboradorIdandDate(Long Id, LocalDateTime Date){
    return this.traslados.stream().filter(x -> x.getColaboradorID().equals(Id) && x.getFechaCreacion().equals(Date)).toList();
  }
}