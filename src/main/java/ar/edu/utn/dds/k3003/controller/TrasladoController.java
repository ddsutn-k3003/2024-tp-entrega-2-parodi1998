...
public class TrasladoController {
  ...
  public void cambiarEstado(Context context) {
    ...
    try {
      var trasladoDTO = this.fachada.buscarXId(id);
      trasladoDTO.setStatus(context.attribute("status"));
      context.json(trasladoDTO);
  ...
  public void bucarTraslasdosDeColaborador(Context context) {
    var id = context.queryParamAsClass("id", Long.class).get();
    try {
      var traslados = this.fachada.trasladosDeColaborador(id,5,2024);
